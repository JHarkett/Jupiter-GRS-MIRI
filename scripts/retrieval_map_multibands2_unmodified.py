...
working_dir = 'july/east' # Structure should be: [epoch]/[tile]

stage = 0
band_name = 'ch3-medium' # Only needed if stage == 0

dir_in = 'd_combined_nofringe_bkg' # Location of Nyquist dither-combined data

mult_err = 1.0 # Value to multiply spectral uncertainty by (see note below)

remove_pix = True # Set to True to remove spaxels
positions = [140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 151]
...
def convert_TB(flux, wave):
	...
	radiance_out = (flux * 1e-26) # Radiance in W/m2/sr/m-1
	radiance_out = radiance_out * c # Radiance in W/m2/sr/m

	a = np.multiply(c1, v**3)/radiance_out
	TB_out = np.multiply(c2, v)/np.log(a+1)
...
def convert_spx(TB, wave):
	...
	a=(2 * h * c**2)/(l**5)
	b = np.exp(np.multiply(h, c)/(l * kb * TB)) - 1
...
for kk in range(len_b):
	...
	TB = convert_TB(spec_data_noblankt[kk], wave[kk])
	flux[kk] = convert_spx(TB, wave[kk])

	TB_err = convert_TB(spec_err_noblankt[kk], wave[kk])
	flux_err[kk] = convert_spx(TB_err, wave[kk])

	flux[kk][np.where(np.isnan(flux[kk]) == True)] = 0.0
	flux_err[kk][np.where(np.isnan(flux_err[kk]) == True)] = 0.0
